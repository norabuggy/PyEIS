...
This script contains tools for extracting impedance data from data files. Currently following data
files are supported
    - Bio-Logic '.mpt' files
    - Gamry's '.DTA' files
...
__all__ = ['correct_text_EIS', 'extract_dta', 'extract_mpt', 'extract_solar']
...
def extract_mpt(path, EIS_name):
    """
    Extracting PEIS and GEIS data files from EC-lab '.mpt' format, coloums are renames following 
    correct_text_EIS()
    
    Kristian B. Knudsen (kknu@berkeley.edu || kristianbknudsen@gmail.com)
    """
    # findes line that states skiplines
    EIS_init = pd.read_csv(path + EIS_name, sep='\t', nrows=1, header=0, names=['err'],
                           encoding='latin1')
    # locates number of skiplines
    EIS_test_header_names = pd.read_csv(path + EIS_name, sep='\t',
                                        skiprows=int(EIS_init.err[0][18:-1]) - 1, encoding='latin1')
    names_EIS = []
    for j in range(len(EIS_test_header_names.columns)):
        # reads coloumn text
        names_EIS.append(correct_text_EIS(EIS_test_header_names.columns[j]))
    return pd.read_csv(path + EIS_name, sep='\t', skiprows=int(EIS_init.err[0][18:-1]),
                       names=names_EIS, encoding='latin1')
...
def extract_dta(path, EIS_name):
    ...
    ZC = pd.Index(init.A)
    header_loc = ZC.get_loc('ZCURVE') + 1
    # locates number of skiplines
    ...
    for j in range(len(header_names_raw.columns)):
        # reads coloumn text
        header_names.append(correct_text_EIS(header_names_raw.columns[j]))
    data = pd.read_csv(path + EIS_name, sep='\t', skiprows=ZC.get_loc('ZCURVE') + 3,
                       names=header_names, encoding='latin1')
...
def extract_solar(path, EIS_name):
    """
    Extracting data files from Solartron's '.z' format, coloums are renames following 
    correct_text_EIS()
    ...
    for j in range(len(header_names_raw.columns)):
        # reads coloumn text
        header_names.append(correct_text_EIS(header_names_raw.columns[j]))
    data = pd.read_csv(path + EIS_name, sep='\t', skiprows=header_loc + 2, names=header_names,
                       encoding='latin1')
